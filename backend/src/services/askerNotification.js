 ...
 * @returns {string}
 */
function buildAnswerUrl(wpPostId) {
  const baseUrl = (process.env.WP_SITE_URL || process.env.WP_API_URL || '')
    .replace(/\/wp-json.*$/, '')
    .replace(/\/$/, '');
  return `${baseUrl}/question/${wpPostId}`;
}
...
async function notifyAskerNewAnswer(questionId) {
  ...
  const answerUrl = data.wp_post_id ? buildAnswerUrl(data.wp_post_id) : '';
    ...
    try {
      await sendEmail(
        ...
        `<div dir="rtl" style="font-family: Arial, sans-serif;">
          <h2>שלום,</h2>
          <p>שמחים לבשר לך שהרב ${data.rabbi_name} ענה על שאלתך.</p>
          ${answerUrl
